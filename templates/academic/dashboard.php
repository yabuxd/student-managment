...
<body>
    ...
    <script>
        ...
        async function viewClassRoster(sectionId, sectionName) {
            ...
            const modalHtml = `
                <h2 style="font-size: 1.8rem; font-weight:700; color:#2c3e50; margin-top:0;">Student Roster - Class ${sectionName}</h2>
                <div class="academic-table-container">
                    <table class="academic-table">
                        <thead>
                            <tr>
        ...
        async function viewClassAssessments(assignmentId, sectionId, className) {
            ...
            const modalHtml = `
                <h2 style="font-size: 1.8rem; font-weight:700; color:#2c3e50; margin-top:0;">Assessments</h2>
                <p style="font-family:'Inter'; font-size:0.95rem; color:#546e7a; margin-top:-0.5rem; margin-bottom:1.5rem;">Class Section: ${className}</p>

                <div class="academic-card" style="background: #fafafa; padding: 1.5rem; margin-bottom: 1.5rem; border-top-color:#e0e0e0;">
                    <h4 style="margin-top:0; font-size:1.1rem; font-weight:700; color:#2c3e50; text-transform:uppercase; letter-spacing:0.05em;">Add assessment ledger</h4>
                    <form onsubmit="handleCreateAssessment(event, ${assignmentId}, ${sectionId}, '${className}')" style="display:grid; grid-template-columns: 2fr 1fr 1.5fr 1fr; gap:0.5rem; align-items:center;">
                        <input type="text" id="newAssTitle" class="academic-input" style="margin-bottom:0;" required placeholder="Assessment Title (e.g. Essay 1)">
                        <input type="number" id="newAssMax" class="academic-input" style="margin-bottom:0;" required placeholder="Max Score" min="1" max="100">
                        <select id="newAssType" class="academic-input" style="margin-bottom:0; height:auto;" required>
                            ${res.assessment_types.map(t => `<option value="${t.id}">${t.name} (${t.weight * 100}%)</option>`).join('')}
                        </select>
                        <button type="submit" class="academic-btn" style="padding: 0.8rem 1rem;">CREATE</button>
                    </form>
                </div>
