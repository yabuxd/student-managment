...
<body>
    <div class="container">
        <nav style="display: flex; justify-content: space-between; align-items: center; padding: 1.5rem 0; border-bottom: 4px solid var(--border-dark); margin-bottom: 2rem;">
            <a href="#" style="font-size: 1.5rem; font-weight: 900; text-decoration: none; color: var(--text-color, inherit); text-transform: uppercase;">⚡ <?php echo htmlspecialchars($brandName); ?></a>
            <div style="display: flex; gap: 1rem;">
                <a href="/login.php" style="font-weight: 700; text-decoration: none; color: var(--text-color, inherit); align-self: center;">LOGIN</a>
                <a href="/register.php" class="brutal-btn" style="padding: 0.5rem 1rem; background: var(--bg-card, #fff); color: var(--text-color, #000); text-decoration: none; display: inline-block;">START FREE</a>
            </div>
        </nav>

        <div style="text-align: center; margin-top: 6rem; position: relative;">
            <div style="position: absolute; top: -2rem; left: 10%; background: var(--accent-1, #0df); padding: 0.5rem 1rem; border: 4px solid var(--border-dark); transform: rotate(-10deg); font-weight: 900; text-transform: uppercase; box-shadow: 4px 4px 0 var(--border-dark); color: #000;">Radical Education</div>
            
            <h1 style="font-size: 6rem; line-height: 0.9; margin-bottom: 2rem; text-transform: uppercase; letter-spacing: -0.05em; font-weight: 900; color: var(--text-color, #000);">
                <?php echo htmlspecialchars($heroTitle); ?>
            </h1>
            <p style="font-size: 1.5rem; font-weight: 600; max-width: 700px; margin: 0 auto 3rem; line-height: 1.4; color: var(--text-muted, #333);">
                <?php echo htmlspecialchars($heroSubtitle); ?>
            </p>
            <div style="display: flex; justify-content: center; gap: 1.5rem;">
                <a href="/login.php" class="brutal-btn" style="padding: 1rem 2rem; background: var(--border-dark); color: var(--bg-color, #fff); text-decoration: none; display: inline-block; font-size: 1.2rem;">ENTER PORTAL</a>
                <a href="/register.php" class="brutal-btn" style="padding: 1rem 2rem; background: var(--accent-3, #f05); color: var(--text-color, #000); text-decoration: none; display: inline-block; font-size: 1.2rem;">GET ENROLLED</a>
            </div>
        </div>

        <div style="margin-top: 8rem; margin-bottom: 6rem; border-top: 4px solid var(--border-dark); border-bottom: 4px solid var(--border-dark); padding: 1rem 0; background: var(--accent-2, #ff0); overflow: hidden; white-space: nowrap; color: #000;">
        ...
        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 2rem; margin-bottom: 6rem;">
            <div class="brutal-card" style="background: var(--accent-1, #0df); padding: 2.5rem; color: #000;">
            ...
            <div class="brutal-card" style="background: var(--accent-2, #ff0); padding: 2.5rem; color: #000;">
            ...
            <div class="brutal-card" style="background: var(--accent-3, #f05); padding: 2.5rem; color: #000;">
